...
import pymysql
from etc.conf import DB_INFO_SRC
...
def mysql_decorator(func):
    def connection(*args, **kwargs):
        database_info = kwargs.get('database_info', DB_INFO_SRC)
        database_info = DB_INFO_SRC if mininet.iteration_is_none(database_info) else database_info
...
class MySQLManage(object):
    ...
    @mysql_decorator
    @retry(wrap_exception=True, stop_max_attempt_number=stop_max_attempt_number, stop_max_delay=stop_max_delay)
