...
# # 一行行往下找
# f_name = 'data.txt'
# with open(f_name, 'r', encoding='utf-8') as f:  # 打开文件
...
def read_afters(file_name, offset=-50, row=1):
    """
    返回文件最后row行
    f_name: 文件名
    offset: -50  # 设置偏移量, 正往结束方向移动，负往开始方向移动。
    ...
    with open(file_name, 'rb') as f:  # 打开文件
        ...
        while True:
            try:
                ...
                f.seek(offset, 2)  # seek(offset, 2)表示文件指针：从文件末尾(2)开始向前50个字符(-50)
                lines = f.readlines()  # 读取文件指针范围内所有行
                if len(lines) >= row + 1:  # 判断是否最后至少有两行，这样保证了最后一行是完整的
                    last_line = lines[-row:]  # 取最后一行
                    break
                # 如果off为50时得到的read_lines只有一行内容，那么不能保证最后一行是完整的
                # 所以off翻倍重新运行，直到read_lines不止一行
                offset *= 2
...
if __name__ == "__main__":
    print(read_afters('data.txt', row=2))
