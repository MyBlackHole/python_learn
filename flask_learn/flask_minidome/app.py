...
from flask import render_template, request, redirect, url_for, session, Response, make_response, jsonify
from flask.views import MethodView
from mixin import LoginRequiredMixin
from models import User, EmpInfo
...
class AddEmp(LoginRequiredMixin, MethodView):
    ...
    def post(self, page):
        ...
        empinfo = EmpInfo(name=name, salary=salsry, age=age, operation=filename.hexdigest())
        db.session.add(empinfo)
        db.session.commit()
        pag = db.session.query(EmpInfo).paginate(page=page, per_page=4)
