...
def validate_marketing_requirement(user):
    ...
    order = Orders.objects.filter(user=user,
                                  status__in=[Orders.HAS_PAID, Orders.RECEIVING, Orders.DONE, Orders.SERVING]) \
...
def order_cancel(order):
    ...
    if order.asset_pay > 0 or order.recharge_pay > 0:
        WxUserAccountLog.record(order.user, WxUserAccountLog.USE_RETURN, asset=order.asset_pay,
                                balance=order.recharge_pay, remark='取消订单退还', note=f'订单:{order.order_sn}',
                                number=order.order_sn)
    if order.credit > 0:
        WxUserCreditLog.record(order.user, WxUserCreditLog.REPLACEMENT_RETURN, credit=order.credit,
                               number=order.order_sn,
                               remark='订单未支付返还积分')
    order.save()
