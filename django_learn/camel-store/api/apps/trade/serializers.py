...
class GoodsBackupSerializers(serializers.ModelSerializer):
    ...
    class Meta:
        model = GoodsBackup
        fields = generate_fields(GoodsBackup, add=['image', 'goods_url'],
                                 remove=['g_image', 'g_rebate', 'g_bonus', 'share_user_id',
                                         'order', 'gtype_id', 'url'])
...
class ItemsSerializers(serializers.HyperlinkedModelSerializer):
    ...
    class Meta:
        model = Items
        fields = generate_fields(Items,
                                 add=['order_sn', 'add_time', 'order_id', 'zh_send_type', 'send', 'arrive', 'confirm',
