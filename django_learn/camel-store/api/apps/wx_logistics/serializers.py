...
class AddOrderSerializer(serializers.ModelSerializer):
    sender = serializers.PrimaryKeyRelatedField(label='发货人信息', queryset=Sender.objects.all(), required=True,
                                                allow_null=False)
    ...
    custom_remark = serializers.CharField(label='备注', style={'base_template': 'textarea.html'}, required=False,
                                          allow_null=True)
    ...
    def validate_receiver(self, data):
        if not data.get('address', None) or not data.get('area', None) or not data.get('city', None) or not data.get(
                'province', None):
