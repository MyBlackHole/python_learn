...
class selectSearch extends Component {
    ...
    render() {
        ...
        const datagroup =
            data.length === 0
                ? datalist.map(d => < Option key = {d.url}
        value = {d.url} > {d.name || d.label} < /Option>)
    :
        data.map(d => < Option
        key = {d.url}
        value = {d.url} > {d.name || d.label} < /Option>);
        const _extra = extra
            ? extra.map(item => < Option key = {item.key}
        value = {item.name || item.label} > {item.name || item.label} < /Option>)
    :
        null;
