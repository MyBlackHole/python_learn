...
class BodyRow extends Component {
    render() {
        ...
        return (
            < span
        className = {isSelect ? `${styles[className]} ${styles.unmultiOpera}` : styles.unmultiOpera} >
            < div
    ...
    <
        span
        className = {styles.Itemcheckbox} > < Checkbox
        checked = {isSelect}
        onChange = {()
    =>
        handleItem(item)
    ...
    <
        div
        className = {styles.title_text} > {item.label} < /div>
            < div
        className = {styles.title_time} >
...
class SourceImage extends Component {
    ...
    render() {
        ...
        handleItem = {(item)
    =>
        restprops && restprops.onSelectItem(item)
    }
...
class sourceImageTab extends Component {
    ...
    render() {
        ...
        const {
            visible, selectModal, content, _tag_list, taglist, uploadList, zipuploadList, check_all,
            tagSelectvisible, Itemtaglist,
            page, pageSize, uploadform, selectList, request
        } = this.state
    ...
    <
        div
        className = {styles.sliderTitle} > 标签 < /div>
            < div
        className = {styles.sliderTag} >
            < span
        className = {taglist.filter(item => item.check).length === 0 ? styles.tagcheck : null} >
            < Tag
        onClick = {()
    =>
        this.selectTag(null, 'all')
    }>
        全部 < /Tag>
        < /span>
        {
            ...
            < span
            className = {item.check ? styles.tagcheck : null} >
                < Tag
            closable
            afterClose = {()
        =>
            this.handleTag(item, 'delete')
        }>
        <
            span
            onClick = {()
    ...
    <
        ...
        {
            resultlist.length > 0 && < Pagination
            {...
                pagination
            }
            /> }
        ...
        =>
            ...
            restprops = {
            {
                text: `选择${typeText[type]}压缩包`, listType
            :
                'text', accept
            :
                '.zip', showUploadList
            ...
            help = {`支持${type === 'picture' ? 'jpeg、jpg、png' : 'mp4'}格式，大小不超过${type === 'picture' ? '2' : '10'}MB。数量不超过5。`
        }>
        <
            UploadImage
            limit = {5}
        ...
        =>
            ...
            restprops = {
            {
                accept: type === 'picture' ? '.jpg,.jpeg,.png' : '.mp4', text
            :
                '选择视频'
            }
        }
            ...
            onCancel = {()
        =>
            this.setState({tagSelectvisible: false, itemTag: {}, Itemtaglist: []})
        }
            onOk = {()
        =>
            this.changeTag()
        ...
        >
            ...
            标签：
          {
              ...
              onChange = {()
          =>
              this.checkhandleTag(item.url, 'item')
          }>
              {
                  item.content
              }
