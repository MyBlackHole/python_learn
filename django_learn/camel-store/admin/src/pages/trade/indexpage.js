...
class indexPage extends Component {
    ...
    handleOk = (type) => {
        ...
        if (flag && shortflag) {
            ...
            const payload = id ? {type, id, data: {..._data}} : {
                type,
                _params: {index: homepage[type].length + 1, ..._data}
            }
            dispatch({
                type: _type,
                payload: {
    ...
    parthandle = (record) => {
        ...
        Modal.confirm({
            title: '确定操作',
            content: ( < Fragment > 确定 < span style = {
        {
            color:'red'
        }
    }>
    ...
    <
        ...
        title = "首页轮播图设置"
        extra = {homepage.homebanner.length <= 4 && < Button type = "primary" size = "small" onClick = {()
    =>
    ...
    =>
        ...
        onOk = {()
    =>
        this.state.request && this.setState({request: false}, () => this.handleOk('homebanner'))
    }
        onCancel = {()
    =>
    ...
    =>
        ...
        handleSource = {()
    =>
        this._openModal(!modalData.image)
    }
        restprops = {
        {
        ...
        onChange = {(e)
    =>
        this.handlelevel('jump_type', e)
    }>
    <
        Option
        key = "goods" > 商品详情 < /Option>
            < Option
        key = "goods_type" > 商品类型 < /Option>
            < Option
        key = "category" > 商品分类 < /Option>
            < /Select>
            < /FormItem>
            < FormItem
        label = "跳转内容" >
    ...
    =>
        ...
        className = {styles.main}
        title = "快速入口设置"
        extra = {homepage.shortcut.length <= 7 && < Button type = "primary" size = "small" onClick = {()
    =>
    ...
    =>
        ...
        onOk = {()
    =>
        this.state.request && this.setState({request: false}, () => this.handleOk('shortcut'))
    }
        onCancel = {()
    =>
        this.setState({entervisible: false})
    }
    >
    <
        Spin
    ...
    <
        ...
        autoplay >
        {
            homepage.homebanner.map(item =>
                < div > {item.image && < img src = {item.image.file} style = {
        {
            width: '100%', height
        :
            '150px'
    ...
    <
        /Row>
        {
            this.verifyModule('grouping') && < img
            src = {groupbuy}
            width = "100%" / > || null
        }
        {
            this.verifyModule('periodic') && < img
            src = {delivery}
            width = "100%" / > || null
        }
        {
            this.verifyModule('recommendation') && < img
            src = {recommend}
            width = "100%" / > || null
        }
        {
            this.verifyModule('credit') && < img
            src = {integrate}
            width = "100%" / > || null
        }
    <
    ...
    <
        ...
        onSelectItem = {(list)
    =>
        this.setState({_selectList: list})
    }
        {...
            this.props
        }
