...
:
    100,
        render
:
    (t, r) => `￥${r.goods_info && r.goods_info.price.toFixed(2)}`,
}
,
{
    title: '数量',
...
:
    'num',
        render
:
    (t, r) => r.goods_info && r.goods_info.num,
}
,
{
    title: '价格',
...
:
    100,
        render
:
    (t, r) => `￥${r.goods_info && r.goods_info.order_amount.toFixed(2)}`,
}
,
{
    title: '订单号',
...
ladderLevel = () => {
    ...
    const dlevel = dvalue ? ( < span style = {
    {
        color:'orange'
    }
}>
    ...
    if (orderdata.status === 'build') {
        return
    <
        Fragment > {levelindex ? `阶梯${levelindex}` : '无'}
        {
            dlevel
        }
    <
...
<
    ...
    :
        {
            id: orderdata.owner && orderdata.owner.id
        }
    }
}>
<
