...
<
    ...
    <
        Fragment >
        单价：{
        goodtype === 'repl' ? `${r.goods_backup.price}积分` : `￥ ${r.goods_backup.price}`
    }
    <
        br / >
        数量：{
        ...
        件 < br / >
        总价：{
        goodtype === 'repl' ? `${(r.goods_backup.price * r.goods_backup.num)}积分` : `￥ ${(r.goods_backup.price * r.goods_backup.num).toFixed(2)}`
    }
    <
        /Fragment>
    }
...
<
    Button > {r.zh_send_type} < /Button></s
    pan >
...
=>
    this.tableHandle('send_type', 'receiving')
}>
    确认发货
    < /Button>
) :
...
<
    ...
    const otherMessage = [
        ...
        title = {this.parseAttach(r.goods_backup.attach).map(item => < div > {item.label}
:
    {
        item.attach_type === 'checkbox' && item.value ? item.value.join(' / ') : item.value
    }
<
    /div>)}
    arrowPointAtCenter >
...
:
    (t, r) =>
        r.edit ? (
            < Select
            disabled = {
    !(orderdata_item_form.send_type === 'receiving')
}
    value = {orderdata_item_form.express_company}
    onChange = {e
=>
    this.tableHandle('express_company', e)
}
    style = {
    {
        width: 120
...
=>
    ...
    编辑 < /a>
    {
        ...
        onClick = {()
    =>
        this.checkOrderData(r.logistics)
    }>
        查看物流 < /a>
        < /Fragment>
    ) :
...
<
    ...
    const logistics_data = logistics_form.map(item => (
        < div
    className = {styles.msgplane_span_msg} >
        < span
    className = {styles.msg_time} > {item.time} < /span>
        < span
    className = {styles.msg_context} > {item.context} < /span>
        < /div>
))
    ;
...
=>
    ...
    onOk = {()
=>
    this.setState({logistics_v: false})
}
    onCancel = {()
=>
    this.setState({logistics_v: false})
