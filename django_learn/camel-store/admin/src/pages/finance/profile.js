...
setid = (record) => {
    ...
    if (record.detail) {
        ...
        this.props.history.push({
            pathname: path,
            query: {id: record.detail.link && record.detail.link.split('/').reverse()[1]}
        });
    }
...
<
    ...
    className = {styles.searchitem} >
        < span
    className = {styles.searchitem_span} > 微信用户 < /span>
        < Input
...
=>
    ...
    className = {styles.searchitem} >
        < span
    className = {styles.searchitem_span} > 返利时间
: <
    /span>
    < RangePicker
    value = {
        accountform
        .date_after ? [moment(accountform.date_after, 'YYYY-MM-DD'), moment(accountform.date_before, 'YYYY-MM-DD')] : null
    }
    ...
    className = {styles.searchitem} >
        < span
    className = {styles.searchitem_span} > 类型
: <
    /span>
    < Select
    style = {
...
<
    Option
    key = {'share'} > 分享返利 < /Option>
        < Option
    key = {'grouping'} > 拼团积分 < /Option>
        < Option
    key = {'replace'} > 积分消费 < /Option>
        < Option
    key = {'gift'} > 店铺赠送 < /Option>
        < Option
    key = {'deduction'} > 店铺扣减 < /Option>
        < /Select>
        < /div>
        < /Fragment>}/ >
        < /Spin>
