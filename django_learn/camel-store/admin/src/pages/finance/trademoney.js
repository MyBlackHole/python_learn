...
:
    80,
        render
:
    (t, r) => ( < span > {statusgroup[r.status]} < /span> )
}
,
{
    title: '操作',
...
:
    ...
    onClick = {()
=>
    this.statusChange(r.user_info.nickname, r.id, 1)
}>
    已提现 < /a>
    < Divider
    type = "vertical" / >
    ...
    onClick = {()
=>
    this.statusChange(r.user_info.nickname, r.id, 2)
}>
    提现失败 < /a>
    < /Fragment>
:
...
<
    ...
    className = {styles.searchitem} >
        < span
    className = {styles.searchitem_span} > 微信号
: <
    /span>
    < Input
...
<
    ...
    className = {styles.searchitem} >
        < span
    className = {styles.searchitem_span} > 状态
: <
    /span>
    < Select
...
<
    ...
    className = {styles.searchitem} >
        < span
    className = {styles.searchitem_span} > 提现时间
: <
    /span>
    < RangePicker
    value = {
        tradeform
        .date_after ? [moment(tradeform.date_after, 'YYYY-MM-DD'), moment(tradeform.date_before, 'YYYY-MM-DD')] : null
    }
...
=>
    ...
    onOk = {()
=>
    !this.state.loading && this.setState({loading: true}, () => this.handleOk())
}
    onCancel = {()
=>
