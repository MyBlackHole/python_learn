...
class EditStore extends Component {
    ...
    render() {
        ...
        title = {id ? '编辑门店' : '新增门店'}
        extra = {[ < Link
        to = '/setting/storelist' > < Button > 取消 < /Button></
        Link >,
    <
        Button
        type = 'primary'
        ...
        onChange = {(e)
    =>
        this.handleStoreform('name', e.target.value)
    }
        />
        < /FormItem>
        < FormItem
        ...
        onChange = {(e)
    =>
        this.handleStoreform('address', e)
    }
        location = {location}
        />
        < Input
        ...
        onChange = {(e)
    =>
        this.handleStoreform('detail', e.target.value)
    }
        />
        < Button
        onClick = {this.converhandle}
        type = "primary" > 转换坐标 < /Button>
            < /FormItem>
            < FormItem
        label = "门店坐标"
    ...
    <
        ...
        onChange = {(e)
    =>
        this.handleStoreform('delivery_divide', e)
    }
    >
    <
        Option
        value = "unlimit" > 不限 < /Option>
            < Option
        value = "radius" > 半径划分 < /Option>
            < Option
        value = "geometry" > 精确划分 < /Option>
            < /Select>
            < /FormItem>
            < FormItem
        label = "服务半径（米）"
        style = {storeform.delivery_divide === 'radius' ? {} : {display: 'none'}} >
            < Input
        placeholder = "服务半径"
        style = {
        {
    ...
    =>
        ...
        < FormItem
        label = "门店配送区域"
        style = {storeform.delivery_divide === 'geometry' ? {} : {display: 'none'}} >
            < AreaView
        edit = {
        !!id
    }
        ...
        onChange = {(e)
    =>
        this.handleStoreform('delivery_range', e)
    }
        />
        共
        {
            storeform.delivery_range.length
        }
        个点 < a >（双击可删除节点）<
        /a>
        < /FormItem>
    ...
    <
        ...
        onChange = {(e)
    =>
        this.handleStoreform('entrust', e)
    }
    >
    <
        Option
        ...
        value = {null} > 无 < /Option>
        {
            ...
            onChange = {(e)
        =>
            this.handleStoreform('status', e)
        }
        >
        <
            Option
            value = "open" > 营业中 < /Option>
                < Option
            value = "close" > 休息中 < /Option>
                < /Select>
                < /FormItem>
                < FormItem
            label = "门店管理员"
            ...
            onChange = {(e)
        =>
            this.handleStoreform('admin', e)
        }
        >
            {
                adminlist.map(item => < Option
                key = {item.url} > {item.username} < /Option>)}
                    < /Select>
                    < /FormItem>
                    < FormItem
                label = "客服电话"
