...
const FormItem = props => ( < Form.Item
...
class EditUser extends Component {
        ...
        if (!type) {
            ...
            switch ((record.detail && record.detail.model_type) || record.a_type || record.log_type) {
                ...
                case 'qrpay':
                    ...
                    break;//线下支付
                default:
                    pathname = record.detail && orderType[record.detail.model_type].detail || null;
                    break;
            }
    ...
    render() {
        const {
            userdata, userform, creditLogs, cred_columns, creditCount, cred_page,
            accountLogs, acc_columns, accountCount, acc_page, amount, visible, action, key, relList
        } = this.state
    ...
    >
        {
            ...
            to = {
            {
                pathname: `/user/userlist/edituser`, query
            :
                {
                    id: userdata.referrer.id
                }
            }
        }>
            {
                userdata.referrer.avatar_url && < img
    ...
    <
        ...
        onClick = {()
    =>
        this.toDetailPage(userdata, 'teamuser')
    }>
        查看 < /Button>
        < /FormItem>
        < FormItem
    ...
    =>
        ...
        onClick = {()
    =>
        this.handlePrice('subtract', 'wallet')
    }>
        扣减 < /Button>
        < /FormItem>
        < FormItem
    ...
    <
        span > ￥
        {
            userdata.asset && userdata.asset.toFixed(2)
        }
    <
        /span>
        < /FormItem>
    ...
    =>
        ...
        onClick = {()
    =>
        this.handlePrice('subtract', 'credit')
    }>
        扣减 < /Button>
        < /FormItem>
        < FormItem
    ...
    >
    <
        span > {userdata.order_count && userdata.order_count.ord_order_count}
        个 < /span>
        < Button
        size = "small"
        type = "primary"
        onClick = {()
    =>
        this.toDetailPage(userdata, 'ord')
    }>
        查看 < /Button>
        < span
        style = {
        ...
        /span>
        {
            userdata.order_count && userdata.order_count.sub_order_count
        }
        个
        < span > < Button
        size = "small"
        ...
        onClick = {()
    =>
        this.toDetailPage(userdata, 'sub')
    }>
        查看 < /Button></s
        pan >
        < /FormItem>
        ...
        <
            ...
            onChange = {(e)
        =>
            this.handleModalUser(e, 'bonus_right')
        }
            />
            < /span>
            < /FormItem>
    ...
    <
        ...
        onChange = {(e)
    =>
        this.handleModalUser(e, 'testers')
    }
        />
        < /span>
        < /FormItem>
    ...
    <
        ...
        onChange = {(e)
    =>
        this.handleModalUser(e, 'upload_perm')
    }
        />
        < /span>
        < /FormItem>
