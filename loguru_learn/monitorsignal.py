...
def time_monitor(name, address, warning_url, send):
    while True:
        try:
            ...
            body = name + "异常"
            project = name + "监控 - " + address
            if send.last_send_time < datetime.now() + timedelta(minutes=-30) and datetime.now().hour >= 8:
                url = '{0}/Warning/BugWarning?Subject={1}&body={2}&Project={3}'.format(warning_url,
...
class EmailMonitoring(object):
    ...
    def run(self):
        Thread(target=run_monitor, args=(self.name, self.address, self.warning_url)).start()
        if self.send:
            if not hasattr(self.send, 'last_send_time'):
                raise Exception(f' send：必须具有 last_send_time 属性 ')
            Thread(target=time_monitor, args=(self.name, self.address, self.warning_url)).start()
